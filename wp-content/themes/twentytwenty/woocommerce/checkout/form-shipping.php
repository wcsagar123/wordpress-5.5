...
//                echo '</pre>';
                ...
                foreach ($fields as $key => $field) {
                    ...
                    if ($key == 'shipping_state') {
                        ...
                        foreach ( $billing_fields as $bill_key => $bill_field ) {
                            if ($bill_key == 'billing_email') {
                                unset($field['class'][0]);	
                                //echo '<div class="row"><div class="col-md-10 col-sm-12 col-12">';
                                custom_woocommerce_form_field( $bill_key, $bill_field, $checkout->get_value( $bill_key ) );
                                //echo '</div></div>';
                            }
                        }
                    }elseif ($key == 'shipping_contry') {
                        custom_woocommerce_form_field($key, $field, $contry);
                    }elseif ($key == 'shipping_etage') {
                        custom_woocommerce_form_field($key, $field, $contry);
                    } else {
    ...
    <?php if (apply_filters('woocommerce_enable_order_notes_field', 'yes' === get_option('woocommerce_enable_order_comments', 'yes'))) : ?>
        ...
        <div class="woocommerce-additional-fields__field-wrapper">
            <?php foreach ($checkout->get_checkout_fields('order') as $key => $field) : ?>
                ...
                <?php custom_woocommerce_form_field($key, $field, $checkout->get_value($key)); ?>
