 ...
 * @return array
 */
function news_vibe_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'news-vibe-selected-sidebar', true );
	  	$sidebar = ! empty( $sidebar ) ? esc_attr($sidebar) : 'sidebar-1';
	}
...
add_filter( 'body_class', 'news_vibe_body_classes' );
